...
// const Chatbot1 = ({ isSpeakerActive, toggleSpeaker, speakText, toggleSourcePopup, isLoading, setIsLoading, messages, onSendMessage }) => {
...
//     // Function to handle the opening of the quiz popup
//     setIsQuizOpen(true);
//     setApiUrl("https://mba.ptit.edu.vn/api/random-questions?topic=khdltkd"); // Set the API URL when opening the quiz
//   };
...
//       try {
//         const response = await fetch(`https://mba.ptit.edu.vn/rag_api/tonghop/?time=${timestamp}&q=${inputMessage}&source=khoahocdulieutrongkinhdoanh`, {
//           method: "get",
//           headers: new Headers({
...
//     if (!isSpeakerActive) {
//       speakText(messageText); // chỉ gọi hàm này khi người dùng kích hoạt nút
//     }
//     toggleSpeaker(); // Đảo trạng thái loa
//   };
...
//           />
//           <button
//             onClick={handleOpenQuiz} // Use the handleOpenQuiz for the quiz button
...
//         </div>
//       </div>
//       <QuizPopup isOpen={isQuizOpen} onClose={() => setIsQuizOpen(false)} apiUrl={apiUrl} />
//     </div>
//   );
// };
...
const Chatbot1 = ({
  ...
  const handleOpenQuiz = () => {
    ...
    setApiUrl("https://mba.ptit.edu.vn/api/random-questions?topic=khdltkd");
  };
  ...
  const handleSendMessage = async () => {
    if (inputMessage.trim() !== "" && !isLoading) {
      ...
      try {
        const response = await fetch(`https://mba.ptit.edu.vn/rag_api/rag/?time=${username}&q=${encodeURIComponent(inputMessage)}&source=khoahocdulieutrongkinhdoanh`, {
          method: "GET",
          headers: new Headers({
            "ngrok-skip-browser-warning": "69420",
          }),
