...
const Chatbot15 = ({ isSpeakerActive, toggleSpeaker, speakText, toggleSourcePopup, isLoading, setIsLoading }) => {
  const [messages, setMessages] = useState([
    { id: 1, text: "Hi there!", sender: "user", timestamp: new Date().toLocaleTimeString() },
    { id: 2, text: "Hello! How can I assist you today?", sender: "bot", timestamp: new Date().toLocaleTimeString(), sources: ["Source 1", "Source 2", "Source 3"] },
  ]);
  ...
  const handleSendMessage = async () => {
    if (inputMessage.trim() !== "" && !isLoading) {
      ...
      try {
        const response = await fetch(`https://sculpin-winning-feline.ngrok-free.app/rag/?time=${timestamp}&q=${inputMessage}&source=chatbot15`, {
          method: "get",
          headers: new Headers({
  ...
  try {
    ...
    if (data.explanations && data.explanations[0]) {
      ...
      if (typeof explanation !== 'string') {
        // If explanation is an object, try to extract the text
        if (explanation.text && typeof explanation.text === 'string') {
          explanation = explanation.text;
        } else {
          // Convert the entire explanation object to string
          explanation = JSON.stringify(explanation);
        }
      }
      const parsedText = JSON.parse(explanation);

      // Tách riêng phần "text" và "sources"
      const extractedText = parsedText.response;
      ...
      setMessages((prevMessages) =>
        prevMessages.map((msg) => (msg.id === loadingMessage.id ? botMessage : msg))
      );
    ...
    setMessages((prevMessages) =>
      prevMessages.map((msg) => (msg.id === loadingMessage.id ? errorMessage : msg))
    );
  } finally {
    setIsLoading(false);
  }
